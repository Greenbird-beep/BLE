...
void setup() {
    ...
    pAdvertising->setName("BLE_Server_1"); // Advertised device name
...
void loop() {
    ...
    if (count_cur/2 != count_pre/2) {
      Serial.printf("Sensor triggered. Count: %d\n", count_cur / 2); // Log half-count (pairs of changes)
      count_pre=count_cur;
      ...
      if (pChr && pServer->getConnectedCount()) {
          ...
          pChr->setValue("count: " + std::to_string(count_cur / 2)); // Set new value
          pChr->notify(); // Send notification to client
      }
    }

    pre = cur;       // Save previous sensor state for edge detection
